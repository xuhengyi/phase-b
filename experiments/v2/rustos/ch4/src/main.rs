...
const KERNEL_MAP_END: usize = 0x8600_0000;
...
fn init_tasks() {
    ...
    for index in 0..count {
        ...
        let program_end = align_up(base + PROGRAM_STATIC_SIZE, PAGE_SIZE);
        let stack_top = base + app_slot_size();
        let stack_bottom = stack_top - USER_STACK_SIZE;
        let heap_base = program_end;
        let heap_limit = stack_bottom;
        setup_address_space(index, base, program_end, stack_bottom, stack_top);
...
fn valid_user_range(ptr: usize, len: usize) -> bool {
    ...
    let current = unsafe { CURRENT_TASK };
    let task = unsafe { TASKS[current] };
    (ptr >= task.base && end <= task.program_end)
        || (ptr >= task.heap_base && end <= task.brk)
        || (ptr >= task.stack_bottom && end <= task.stack_top)
}
...
fn setup_address_space(
    ...
    map_user_region(index, stack_bottom, stack_top, PTE_R | PTE_W | PTE_U | PTE_A | PTE_D);
}
...
fn map_user_region(index: usize, start: usize, end: usize, flags: usize) {
    let mut va = align_down(start, PAGE_SIZE);
    let end = align_up(end, PAGE_SIZE);
    unsafe {
        while va < end {
            USER_L0_TABLES.0[index][vpn0(va)] = pte(va, flags);
            va += PAGE_SIZE;
        }
    }
}
