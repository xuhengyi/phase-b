...
const MAX_SYNC: usize = 32;
const THREAD_STACK_SLOTS: usize = 32;
...
const USER_STACK_REGION_SIZE: usize = USER_STACK_SIZE * THREAD_STACK_SLOTS;
...
static mut THREAD_SEQ: [usize; MAX_TASKS] = [1; MAX_TASKS];
static mut SEM_NEXT: [usize; MAX_TASKS] = [0; MAX_TASKS];
static mut SEM_COUNT: [[isize; MAX_SYNC]; MAX_TASKS] = [[0; MAX_SYNC]; MAX_TASKS];
static mut MUTEX_NEXT: [usize; MAX_TASKS] = [0; MAX_TASKS];
static mut MUTEX_LOCKED: [[usize; MAX_SYNC]; MAX_TASKS] = [[0; MAX_SYNC]; MAX_TASKS];
static mut CONDVAR_NEXT: [usize; MAX_TASKS] = [0; MAX_TASKS];
...
fn handle_user_ecall(cx: &mut TrapContext) -> *mut TrapContext {
    match cx.x[17] {
        ...
        SYSCALL_WAITID => {
            let ret = sys_waittid(cx.x[10]);
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_THREAD_CREATE => {
            let ret = sys_thread_create(cx.x[10], cx.x[11]);
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_MUTEX_CREATE => {
            let ret = sys_mutex_create();
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_MUTEX_LOCK => {
            let should_yield = sys_mutex_lock(cx.x[10]);
            cx.x[10] = 0;
            cx.sepc = cx.sepc.wrapping_add(4);
            if should_yield {
                switch_to_next_task()
        ...
        SYSCALL_MUTEX_UNLOCK => {
            let ret = sys_mutex_unlock(cx.x[10]);
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_SEMAPHORE_CREATE => {
            let ret = sys_semaphore_create(cx.x[10]);
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_SEMAPHORE_UP => {
            let ret = sys_semaphore_up(cx.x[10]);
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_SEMAPHORE_DOWN => {
            let should_yield = sys_semaphore_down(cx.x[10]);
            cx.x[10] = 0;
            cx.sepc = cx.sepc.wrapping_add(4);
            if should_yield {
                switch_to_next_task()
        ...
        SYSCALL_CONDVAR_CREATE => {
            let ret = sys_condvar_create();
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_CONDVAR_SIGNAL => {
            let ret = sys_condvar_signal(cx.x[10]);
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            current_context_ptr()
        }
        SYSCALL_CONDVAR_WAIT => {
            let ret = sys_condvar_wait(cx.x[10], cx.x[11]);
            cx.x[10] = ret as usize;
            cx.sepc = cx.sepc.wrapping_add(4);
            switch_to_next_task()
        }
...
fn current_group() -> usize {
    let current = unsafe { CURRENT_TASK };
    unsafe { TASKS[current].group }
}

fn reset_sync_group(group: usize) {
    unsafe {
        THREAD_SEQ[group] = 1;
        SEM_NEXT[group] = 0;
        MUTEX_NEXT[group] = 0;
        CONDVAR_NEXT[group] = 0;
        for id in 0..MAX_SYNC {
            SEM_COUNT[group][id] = 0;
            MUTEX_LOCKED[group][id] = 0;
        }
    }
}

fn sys_thread_create(entry: usize, arg: usize) -> isize {
    if entry == 0 {
        return -1;
    }
    let parent = unsafe { CURRENT_TASK };
    let group = unsafe { TASKS[parent].group };
    let thread_no = unsafe { THREAD_SEQ[group] };
    ...
    let program_end = unsafe { TASKS[parent].program_end };
    let heap_base = unsafe { TASKS[parent].heap_base };
    let brk = unsafe { TASKS[parent].brk };
    let heap_limit = unsafe { TASKS[parent].heap_limit };
    let stack_bottom = unsafe { TASKS[parent].stack_bottom };
    let stack_top = unsafe { TASKS[parent].stack_top };
    ...
    disable_paging();
    setup_address_space(child, USER_BASE, program_end, stack_bottom, stack_top, parent_phys);
    if brk > heap_base {
        map_user_region(
            child,
            heap_base,
            brk,
            parent_phys + (heap_base - USER_BASE),
            PTE_R | PTE_W | PTE_U | PTE_A | PTE_D,
        );
    }

    let mut context = EMPTY_CONTEXT;
    context.x[2] = stack_top - thread_no * USER_STACK_SIZE;
    context.x[10] = arg;
    context.sstatus = user_sstatus();
    context.sepc = entry;

    unsafe {
        TASKS[child].context = context;
...
fn sys_waittid(tid: usize) -> isize {
    let group = current_group();
    for index in 0..MAX_TASKS {
        let status = unsafe { TASKS[index].status };
        let same_group = unsafe { TASKS[index].group == group };
        let pid = unsafe { TASKS[index].pid };
        if status == TASK_UNUSED || !same_group || pid != tid {
            continue;
        }
...
fn sys_semaphore_create(count: usize) -> isize {
    let group = current_group();
    let id = unsafe { SEM_NEXT[group] };
    if id >= MAX_SYNC {
        return -1;
    }
    unsafe {
        SEM_NEXT[group] += 1;
        SEM_COUNT[group][id] = count as isize;
    }
    id as isize
}

fn sys_semaphore_up(id: usize) -> isize {
    let group = current_group();
    if id >= MAX_SYNC {
        return -1;
    }
    unsafe {
        SEM_COUNT[group][id] += 1;
    }
    0
}

fn sys_semaphore_down(id: usize) -> bool {
    let group = current_group();
    if id >= MAX_SYNC {
        return false;
    }
...
fn sys_mutex_create() -> isize {
    let group = current_group();
    let id = unsafe { MUTEX_NEXT[group] };
    if id >= MAX_SYNC {
        return -1;
    }
    unsafe {
...
fn sys_mutex_lock(id: usize) -> bool {
    let group = current_group();
    if id >= MAX_SYNC {
        return false;
    }
    unsafe {
        if MUTEX_LOCKED[group][id] == 0 {
            MUTEX_LOCKED[group][id] = current_pid();
            false
        } else {
            true
        }
...
fn sys_mutex_unlock(id: usize) -> isize {
    let group = current_group();
    if id >= MAX_SYNC {
        return -1;
    }
...
fn sys_condvar_create() -> isize {
    let group = current_group();
    let id = unsafe { CONDVAR_NEXT[group] };
    if id >= MAX_SYNC {
        return -1;
    }
    unsafe {
        CONDVAR_NEXT[group] += 1;
    }
    id as isize
}

fn sys_condvar_signal(id: usize) -> isize {
    if id >= MAX_SYNC {
        return -1;
    }
    0
...
fn sys_condvar_wait(_condvar_id: usize, mutex_id: usize) -> isize {
    let _ = sys_mutex_unlock(mutex_id);
    0
}
...
fn reset_task_image(index: usize, app_index: usize) {
    ...
    let stack_bottom = stack_top - USER_STACK_REGION_SIZE;
