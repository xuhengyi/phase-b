...
const APP_STEP: u64 = 0;
const CH2_BASE_APPS: &[&str] = &[
    "00hello_world",
    "01store_fault",
    "02power",
    "03priv_inst",
...
fn main() {
    ...
    println!("cargo:rerun-if-changed={}", user_root.join("Cargo.toml").display());
    ...
    let mut bins = Vec::with_capacity(CH2_BASE_APPS.len());
    for (index, name) in CH2_BASE_APPS.iter().enumerate() {
        let base = APP_BASE + index as u64 * APP_STEP;
        build_user_app(&user_root, name, base);
...
fn build_user_app(user_root: &Path, name: &str, base: u64) {
